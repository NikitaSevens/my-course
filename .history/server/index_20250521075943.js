...
app.post("/courses", upload.fields([
  ...
  try {
    ...
    const newCourse = {
      id: uuidv4(),
      ...data,
      image: data.image || (files?.imageFile?.[0]?.filename ? `/uploads/${files.imageFile[0].filename}` : ""),
programFile: files?.programFile?.[0]?.filename ? `/uploads/${files.programFile[0].filename}` : "",
