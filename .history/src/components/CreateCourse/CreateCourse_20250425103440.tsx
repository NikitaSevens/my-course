            ...
            {courses.map(course => (
              <div key={course.id} className={styles.courseCard}>
                <div className={styles.courseImage}>
                  {course.image && <img src={course.image} alt="preview" />}
                </div>
                <h3>{course.title}</h3>
                <p>{course.description}</p>
                <p><strong>Дата:</strong> {course.date}</p>
                <button onClick={() => openPopup(course.fullText)}>Смотреть</button>
                <button onClick={() => handleEdit(course.id)}>Изменить</button>
