...
const EnrollPopup = ({ onClose, selectedCourse }: EnrollPopupProps) => {
  ...
  //ДАТА ВЫДАЧИ
  const validatePassportIssueDate = (
    issueDateStr: string,
    birthDateStr: string
  ): string | null => {
    if (!issueDateStr) return "Введите дату выдачи паспорта";
    ...
    if (issueDate < minDate) {
      return "Дата выдачи не может быть раньше 1997 года";
    }
    ...
    if (birthDateStr) {
      ...
      const ageAtIssue =
        ...
        (issueDate < new Date(birthDate.setFullYear(issueDate.getFullYear()))
          ? 1
          : 0);
  
      if (ageAtIssue < 14) {
        return "Паспорт не мог быть выдан ранее 14 лет";
      }
    }
  ...
  <input
    ...
    min={birthDate ? new Date(new Date(birthDate).setFullYear(new Date(birthDate).getFullYear() + 14)).toISOString().split("T")[0] : ""}
