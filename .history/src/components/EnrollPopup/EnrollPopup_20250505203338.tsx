...
const EnrollPopup = ({ onClose, selectedCourse }: EnrollPopupProps) => {
  ...
  const [suggestions, setSuggestions] = useState<any[]>([]); // Список подсказок для адреса
  const [selectedAddress, setSelectedAddress] = useState<string | null>(null); // Выбранный адре
  ...
  type AddressSuggestion = {
    display_name: string;  // Это строка с полным адресом
    lat: number;
    lon: number;
  };
  ...
  const fetchSuggestions = async (query: string) => {
    ...
    setSuggestions(data);  // Устанавливаем подсказки
  };

  const handleChangeSearch = (e: ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setPassportAddress(value);  // Обновляем значение в поле ввода
    fetchSuggestions(value);  // Отправляем запрос на подсказки
  };

  const handleSelect = (address: string) => {
    setPassportAddress(address); // Обновляем поле ввода выбранным адресом
    setSelectedAddress(address);  // Запоминаем выбранный адрес
  ...
  return (
    <div className={styles.overlay}>
      <div className={styles.popup}>
        ...
        Адрес регистрации:
        <input
          value={passportAddress}  // Значение адреса
          onChange={handleChangeSearch}    // Обработчик изменений
          placeholder="Введите адрес" // Текст в поле
        />
      </label>
      ...
      {suggestions.length > 0 && (
        <ul>
          {suggestions.map((suggestion, index) => (
            <li
              key={index}
              onClick={() => handleSelect(suggestion.display_name)} // Выбор адреса
            >
              {suggestion.display_name} {/* Отображение полного адреса */}
            </li>
          ))}
        </ul>
      )}

      {/* Если выбран адрес, показываем его */}
      {selectedAddress && (
        <div>
