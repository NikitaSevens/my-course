...
const EnrollPopup = ({ onClose, selectedCourse }: EnrollPopupProps) => {
  ...
  //КЕМ  ВЫДАН
  interface AhunterSuggestType {
    FMS: {
      Solid: (options: any) => void;
    };
  }
  
  interface Window {
    AhunterSuggest?: AhunterSuggestType;
  }
  useEffect(() => {
    ...
    script.onload = () => {
      if (window.AhunterSuggest && window.AhunterSuggest.FMS) {
        ...
        window.AhunterSuggest.FMS.Solid(options);
      } else {
        console.warn('AhunterSuggest не загрузился');
      }
    };
