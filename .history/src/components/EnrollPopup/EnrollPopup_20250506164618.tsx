...
const EnrollPopup = ({ onClose, selectedCourse }: EnrollPopupProps) => {
  ...
  const fetchPassportSuggestions = async (query: string) => {
    ...
    try {
      ...
      fetchPassportSuggestions(suggestions); // Устанавливаем подсказки для адреса по паспорту
    } catch (error) {
  ...
  const fetchCustomAddressSuggestions = async (query: string) => {
    ...
    try {
      ...
      fetchCustomSuggestions(suggestions); // Устанавливаем подсказки для фактического проживания
    } catch (error) {
      console.error("Ошибка при запросе подсказок для фактического проживания:", error);
    }
  };
