import { useState, useEffect, ChangeEvent, useRe } from "react";
...
const EnrollPopup = ({ onClose, selectedCourse }: EnrollPopupProps) => {
  ...
  const validateSeries = (value: string): string => {
    ...
    // Проверка региона выдачи (первые 2 цифры)
...
function PassportIssuedByField({ docIssuedBy, setDocIssuedBy }) {
      ...
      if (window.AhunterSuggest) {
        window.AhunterSuggest.FMS.Solid({
          ...
          on_choose: function (suggestion) {
            setDocIssuedBy(suggestion.value); // обновляем стейт при выборе
          },
        });
      }
    };
