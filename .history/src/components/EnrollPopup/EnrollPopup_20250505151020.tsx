...
const EnrollPopup = ({ onClose, selectedCourse }: EnrollPopupProps) => {
  ...
  //КЕМ  ВЫДАН
  const ahunter = (window as any).AhunterSuggest;
  useEffect(() => {
    // Загружаем и инициализируем модуль подсказок после рендеринга компонента
    const script = document.createElement('script');
    script.src = 'https://ahunter.ru/suggest/suggest.js';
    script.async = true;

    script.onload = () => {
      if (window.AhunterSuggest) {
        const options = {
          id: 'js-FMSField',
          ahunter_url: 'https://ahunter.ru/', // Веб-адрес сервиса
          empty_msg: '', // Пустое сообщение
          limit: 5, // Показываем только 5 подсказок
          suggest_on_focus: false, // Подсказки не показываются при возврате фокуса
          on_choose: function (suggestion) {
            console.log(suggestion); // Логируем выбранную подсказку
            setDocIssuedBy(suggestion.value); // Обновляем значение в state
          },
        };
