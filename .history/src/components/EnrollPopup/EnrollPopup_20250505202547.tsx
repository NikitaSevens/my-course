  ...
  return (
    <div className={styles.overlay}>
      <div className={styles.popup}>
        ...
        Адрес регистрации:
        <input
          value={passportAddress}  // Значение поля ввода
          onChange={handleChange}    // Обработчик изменений
          placeholder="Введите адрес"  // Подсказка в поле
        />
      </label>

      {/* Если есть подсказки, отображаем их */}
      {suggestions.length > 0 && (
        <ul>
          {suggestions.map((suggestion, index) => (
            <li key={index} onClick={() => handleSelect(suggestion.display_name)}>
              {suggestion.display_name}  {/* Отображаем полный адрес */}
            </li>
          ))}
        </ul>
      )}
