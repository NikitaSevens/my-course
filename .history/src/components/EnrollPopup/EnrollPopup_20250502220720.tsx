...
const EnrollPopup = ({ onClose, selectedCourse }: EnrollPopupProps) => {
  ...
  // Получить максимальную дату (13 лет назад)
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validatePhone = (phoneNumber: string): string | null => {
      ...
      // Проверяем длину (10 цифр без кода страны или 11 с первой 7 или 8)
      if (![10, 11].includes(cleaned.length)) {
        return "Номер должен содержать 10 или 11 цифр";
      }
      
      // Проверяем код страны (если 11 цифр)
      if (cleaned.length === 11 && !/^[78]/.test(cleaned)) {
        return "Код страны должен быть 7 или 8";
      }
      
      // Проверяем код оператора (3 цифры после кода страны)
      const operatorCode = cleaned.length === 10 ? cleaned.substring(0, 3) : cleaned.substring(1, 4);
    ...
    const handleChangeOne = (e: React.ChangeEvent<HTMLInputElement>) => {
      const value = e.target.value;
      // Форматирование ввода: +7 (XXX) XXX-XX-XX
      let formattedValue = value.replace(/\D/g, '');
      
      if (formattedValue.length > 0) {
        formattedValue = formattedValue.match(/.{1,11}/)?.[0] || formattedValue;
        formattedValue = `+7 (${formattedValue.substring(1, 4)}) ${formattedValue.substring(4, 7)}-${formattedValue.substring(7, 9)}-${formattedValue.substring(9)}`;
