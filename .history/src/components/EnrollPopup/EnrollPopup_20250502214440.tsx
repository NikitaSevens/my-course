...
const EnrollPopup = ({ onClose, selectedCourse }: EnrollPopupProps) => {
            ...
            <div className={styles.scrollOne}>
              ...
              <label>
                ...
                <input
                  ...
                  required
                  pattern="^[А-ЯЁ][А-ЯЁа-яё]*(?:-[А-ЯЁа-яё]+)*$"
                  title="С заглавной буквы, исключая [0-9]|[@#$%^&*+=∞№] и [a-z] "
                />
              </label>
              <label>
  Отчество:
  <input
    ...
    onChange={(e) => {
      ...
      // Разрешаем: русские буквы и дефис (для редких случаев двойных отчеств)
      ...
      // Проверка дефиса (не в начале/конце и не повторяется)
      const isValidHyphen = !/^-|-$|--/.test(value);
      
      if (validPattern.test(value) && !hasInvalidChars && isValidHyphen) {
        setPatronymic(value);
      }
    }}
    pattern="^[А-ЯЁ][А-ЯЁа-яё]*(?:-[А-ЯЁа-яё]+)*$"
    title="Отчество должно содержать только русские буквы и дефис (не в начале/конце)"
  />
</label>
              <label>
                Дата рождения:
