...
const EnrollPopup = ({ onClose, selectedCourse }: EnrollPopupProps) => {
  ...
  useEffect(() => {
    ...
    // Загружаем suggest.js
    const loadAhunterSuggest = () => {
      return new Promise<void>((resolve, reject) => {
        const script = document.createElement("script");
        script.src = "https://www.ahunter.ru/js/min/ahunter_suggest.js";
        script.onload = () => resolve();
        script.onerror = () => reject(new Error("AhunterSuggest failed to load"));
        document.head.appendChild(script);
      });
    };
    ...
    const initSuggest = () => {
      if (window.AhunterSuggest && window.AhunterSuggest.FMS) {
        window.AhunterSuggest.FMS.Solid({
          ...
          on_choose: function (suggestion: any) {
            console.log("Подсказка выбрана:", suggestion);
            setDocIssuedBy(suggestion.value);
          },
        });
      } else {
        console.warn("AhunterSuggest is not available");
      }
    };
  
    // Последовательная загрузка и инициализация
    loadJQuery()
      .then(() => loadAhunterSuggest())
      .then(() => initSuggest())
