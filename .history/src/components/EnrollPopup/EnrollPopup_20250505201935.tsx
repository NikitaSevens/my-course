...
const EnrollPopup = ({ onClose, selectedCourse }: EnrollPopupProps) => {
  ...
  //ВАЛИДАЦИЯ ДЛЯ АДРЕСА ПРОЖИВАНИЯ
   ...
   const fetchSuggestions = async (query: string) => {
    if (query.length < 3) return; // Если введено меньше 3 символов, не отправляем запрос

    const response = await fetch(
      `https://nominatim.openstreetmap.org/search?format=json&q=${query}&addressdetails=1&limit=5`
    );
    const data = await response.json();  // Получаем данные в формате JSON
    setSuggestions(data);  // Устанавливаем полученные подсказки в состояние
  };

  // Обработчик изменения значения в поле
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setPassportAddress(value);  // Обновляем состояние адреса
    fetchSuggestions(value);  // Отправляем запрос на получение подсказок
  };

  // Обработчик выбора адреса из подсказок
  const handleSelect = (address: string) => {
    setPassportAddress(address);  // Обновляем значение в поле
