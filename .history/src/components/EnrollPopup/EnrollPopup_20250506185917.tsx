...
const EnrollPopup = ({ onClose, selectedCourse }: EnrollPopupProps) => {
  ...
  const [selectedAddress, setSelectedAddress] = useState<string | null>(null); // Запоминаем выбранный адрес
  const [suggestions, setSuggestions] = useState<string[]>([]); // Список подсказок для адреса
  ...
  const fetchPassportSuggestions = async (query: string) => {
    if (query.length < 3) return; // Только если введено хотя бы 3 символа
    ...
    try {
      const response = await axios.post(
        ...
        {
          headers: {
            Authorization: `Token d879bdf34b1ff6d2c6f1bb0cf5ff0817c6135192`, // Ваш API ключ DaData
          },
        }
      );
      ...
      setPassportSuggestions(suggestions); // Устанавливаем подсказки для адреса по паспорту
    } catch (error) {
      console.error("Ошибка при запросе подсказок для паспорта:", error);
    }
  };

  // Функция для запроса подсказок для фактического проживания
  ...
  <input
    ...
    onChange={(e) => {
      ...
      // Проверка: только русские буквы, пробелы, дефисы
      const onlyRussian = /^[А-ЯЁа-яё\s-]*$/;
      const hasForbidden =
        /[0-9@#$%^&*+=∞№a-zA-Z]|[-]{2,}|^\s|\s$|\s{2,}/.test(value); // запрещённые символы

      if (onlyRussian.test(value) && !hasForbidden) {
        setFullName(value);
      }
    }}
    required
    pattern="^[А-ЯЁ][а-яё]+(?:-[А-ЯЁа-яё]+)?\s[А-ЯЁ][а-яё]+(?:-[А-ЯЁа-яё]+)?\s[А-ЯЁ][а-яё]+(?:-[А-ЯЁа-яё]+)?$"
    title="Введите Фамилию, Имя и Отчество через пробел, с заглавной буквы. Только русские буквы, допустим один дефис."
