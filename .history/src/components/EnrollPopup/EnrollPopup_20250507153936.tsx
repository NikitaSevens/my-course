            ...
            <label>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const onlyRussianLetterss = /^[А-ЯЁа-яё\s-]*$/;

                        // Проверяем количество слов (не более 3)
                        const wordCountt = value
                          .trim()
                          .split(/\s+/)
                          .filter(Boolean).length;

                        if (onlyRussianLetterss.test(value) && wordCountt <= 3) {
                          setParentFio(value);
                        }
                      }}
                      onBlur={(e) => {
                        // Автоматически форматируем при потере фокуса
                        const formatteds = e.target.value
                          .trim()
                          .replace(/\s+/g, " ") 
                          .replace(
                            /(^|\s)([а-яё])/g,
